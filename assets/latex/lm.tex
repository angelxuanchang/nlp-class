...
\begin{itemize}
\item HW0 due today Midnight
\item Do not have spaces or dashes in your group name!!!
\item Make sure your Coursys group name and gitlab repo matches!!!
\end{itemize}
\end{block}
...
Yasaman Etesam & this Fri 3:30-4:30 pm & ASB 9808
\end{tabular}
\end{block}
\end{frame}
\end{comment}
...
\end{itemize}
\pdfcomment{This is also known as the maximum likelihood estimate.  Parameters that maximizes the probability of the training data}
\end{frame}
...
\[ p(clown \mid crazy, killer) = \frac{c(crazy, killer, clown)}{c(crazy, killer)} \]
\end{itemize}
\pdfcomment{This is also known as the maximum likelihood estimate}
\end{block}
\end{frame}
\end{comment}
...
\item Improved JM smoothing, a separate \highlight{$\lambda$ for each $w_{i-1}$}: 
...
\item Need to handle \highlight{data sparsity} \\ use Markov assumption and smoothing
